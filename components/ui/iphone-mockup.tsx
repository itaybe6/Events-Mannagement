...
export interface IPhoneMockupProps {
  ...
  /** When set with fitHeight, root is sized to fit and inner frame is scaled so the whole phone is visible (no clipping). */
  fitWidth?: number;
  fitHeight?: number;
...
export const IPhoneMockup: React.FC<IPhoneMockupProps> = ({
  ...
  const fitScale =
    fitWidth != null && fitHeight != null
      ? Math.min(fitWidth / outerWidth, fitHeight / outerHeight)
      : scale;
  ...
  return (
    <div className={className} style={wrapperStyle}>
      {scaledInnerStyle ? (
        <div style={scaledInnerStyle}>{frameEl}</div>
      ) : (
        frameEl
      )}
    </div>
