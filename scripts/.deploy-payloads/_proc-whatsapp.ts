...
export function parseTableNumber(raw: unknown): number | null {
...
export function buildTableNumberById(rows: Array<{ id: string; number?: unknown }>): Map<string, number> {
  const map = new Map<string, number>();
  for (const row of rows) {
    const n = parseTableNumber(row.number);
    if (n != null) map.set(String(row.id), n);
  }
  return map;
}

export function resolveGuestTableNumberText(guest: any, tableNumberById: Map<string, number>): string {
  const tableId = guest?.table_id ? String(guest.table_id).trim() : "";
  if (!tableId) return "";
  const n = tableNumberById.get(tableId);
  return n != null ? String(n) : "";
}

export async function loadTableNumberMap(adminClient: any, eventId: string): Promise<Map<string, number>> {
  const { data, error } = await adminClient.from("tables").select("id, number").eq("event_id", eventId);
  if (error || !Array.isArray(data)) return new Map();
  return buildTableNumberById(data);
}
...
export function buildGuestVars(args: {
  ...
  const tableNumberText = String(args.tableNumberText ?? "").trim();
  return {
    name: fullName,
    link,
