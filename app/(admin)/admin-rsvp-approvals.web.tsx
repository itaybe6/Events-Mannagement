...
function Metric({
  ...
  label: string;
  value: number;
  tone: 'green' | 'yellow' | 'red' | 'primary' | 'muted';
  total: number;
}) {
  ...
  const valueColor = tone === 'muted' ? colors.gray[500] : colors.text;
  ...
  return (
    <View style={styles.metric}>
      ...
      <View style={styles.metricTrack}>
        <View style={[styles.metricBar, { width: `${Math.round(pct * 100)}%`, backgroundColor: barColor } as any]} />
      </View>
    </View>
  );
}
...
export default function AdminRsvpApprovalsWebScreen() {
  ...
  return (
    <View style={styles.page}>
      ...
      <View style={styles.header}>
        <View style={styles.headerTopRow}>
          <View style={styles.breadcrumbRow}>
            <Text style={styles.breadcrumbText}>אירועים</Text>
            <Ionicons name="chevron-back" size={14} color={colors.gray[500]} />
            <Text style={styles.breadcrumbText} numberOfLines={1}>
              {resolvedEventId ? `מזהה אירוע: ${resolvedEventId}` : 'אירוע'}
            </Text>
          </View>

          <Text style={styles.h1}>ניהול מוזמנים</Text>
        </View>
        ...
        <ScrollView
          ...
          style={{ flexGrow: 0 }}
        >
          <Metric label="מאושרים" value={stats.coming} tone="green" total={stats.total} />
          <Metric label="ממתינים" value={stats.pending} tone="yellow" total={stats.total} />
          <Metric label="דחו" value={stats.notComing} tone="muted" total={stats.total} />
          <Metric label='סה״כ' value={stats.total} tone="primary" total={stats.total} />
        </ScrollView>
      </View>
      ...
      >
        <View style={[styles.toolbar, Platform.OS === 'web' ? styles.toolbarSticky : null]}>
          <View style={styles.toolbarRight}>
            <View style={styles.searchBox}>
              ...
              <TextInput
                ...
                placeholder="חיפוש אורח, טלפון או קבוצה..."
        ...
        {loading ? (
          ...
          <View style={styles.sectionsWrap}>
            {sections.map((sec) => {
              ...
              const cat = categories.find((c) => String(c.name || '').trim() === sec.name);
              const sideLabel = cat?.side ? (cat.side === 'groom' ? 'צד החתן' : 'צד הכלה') : null;
                      ...
                      {sec.data.slice(0, visibleCount).map((g) => {
                        ...
                        return (
                          <Pressable
                            key={g.id}
                            accessibilityRole="button"
                            accessibilityLabel={`אורח ${g.name}`}
                            onPress={() => {}}
                            style={({ hovered }: any) => [
                              styles.guestRow,
                          ...
                          >
                            ...
                            <View style={styles.guestLeft}>
                              ...
                              <View style={styles.rowActions}>
                                <Pressable
                                  accessibilityRole="button"
                                  accessibilityLabel={`שלח הודעה ל${g.name}`}
                                  onPress={() => {}}
                                  style={({ hovered, pressed }: any) => [
                                    styles.iconBtn,
                                ...
                                >
                                  <Ionicons name="chatbubble-ellipses-outline" size={16} color={colors.gray[700]} />
                                </Pressable>
...
const styles = StyleSheet.create({
  ...
  container: { paddingTop: 16, paddingBottom: 28, paddingHorizontal: 16, gap: 14, maxWidth: 1220, alignSelf: 'center', width: '100%' },
  header: {
  ...
  headerTopRow: { gap: 6 },
  breadcrumbRow: { flexDirection: 'row-reverse', alignItems: 'center', gap: 6, flexWrap: 'wrap' },
  breadcrumbText: { fontSize: 11, fontWeight: '800', color: colors.gray[500], textAlign: 'right' },
  h1: { fontSize: 22, fontWeight: '900', color: colors.primary, textAlign: 'right' },

  metricsRow: { flexDirection: 'row-reverse', alignItems: 'center', gap: 18, paddingTop: 10, paddingBottom: 2 },
  metric: { minWidth: 110, paddingHorizontal: 10, borderLeftWidth: 1, borderLeftColor: 'rgba(15,23,42,0.06)' },
  metricLabel: { fontSize: 11, fontWeight: '800', color: colors.gray[500], textAlign: 'center' },
  metricValue: { marginTop: 4, fontSize: 18, fontWeight: '900', textAlign: 'center' },
  metricTrack: { marginTop: 8, height: 4, backgroundColor: 'rgba(15,23,42,0.06)', borderRadius: 999, overflow: 'hidden' },
  ...
  toolbarSticky:
    Platform.OS === 'web'
      ? ({
          ...
          top: 160,
          zIndex: 25,
          boxShadow: '0 2px 10px rgba(11, 27, 65, 0.06)',
        } as any)
      : ({} as any),
  ...
  sectionAccentBar: { width: 8, height: 32, borderRadius: 999, backgroundColor: '#3b82f6' },
  ...
  guestRowHover: { backgroundColor: 'rgba(15,23,42,0.02)' },
  ...
  iconBtnDanger: { backgroundColor: 'transparent', borderColor: 'rgba(15,23,42,0.08)' },
  iconBtnDangerHover: { backgroundColor: 'rgba(239, 68, 68, 0.06)', borderColor: 'rgba(239, 68, 68, 0.20)' },
