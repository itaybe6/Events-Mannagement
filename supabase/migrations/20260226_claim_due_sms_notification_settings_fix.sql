-- Fix claim_due_sms_notification_settings return shape/type mismatches (safe re-run)
--
-- Some environments ended up with `recipient_guest_ids` stored/returned in a shape that doesn't
-- exactly match the function's RETURNS TABLE signature, causing:
--   "structure of query does not match function result type"
--
-- This re-defines the RPC with explicit casts and a robust conversion of recipient_guest_ids
-- into uuid[] regardless of whether the underlying column is uuid[] or text[].
