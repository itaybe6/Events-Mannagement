-- Scheduled messages must never be starved by leftover catch-up jobs.
-- Already-claimed (setting, scheduled_for) pairs are excluded before LIMIT.
-- Mode:
--   scheduled = only the original notification_date
--   catchup   = only the late-guest queue
--   all       = scheduled first, then catchup with leftover slots (cron compat)
...
CREATE OR REPLACE FUNCTION public.claim_due_scheduled_notification_settings(p_limit integer DEFAULT 25)
RETURNS TABLE (
  run_id uuid,
  setting_id uuid,
  event_id uuid,
...
SET search_path = public
AS $$
  SELECT * FROM public.claim_due_notification_jobs(p_limit, 'scheduled');
$$;

CREATE OR REPLACE FUNCTION public.claim_due_catchup_notification_settings(p_limit integer DEFAULT 25)
RETURNS TABLE (
  run_id uuid,
  setting_id uuid,
  event_id uuid,
...
SET search_path = public
AS $$
  SELECT * FROM public.claim_due_notification_jobs(p_limit, 'catchup');
$$;
...
SET search_path = public
AS $$
  SELECT * FROM public.claim_due_notification_jobs(p_limit, 'all');
$$;

REVOKE ALL ON FUNCTION public.claim_due_notification_jobs(integer, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.claim_due_scheduled_notification_settings(integer) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.claim_due_catchup_notification_settings(integer) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.claim_due_sms_notification_settings(integer) FROM PUBLIC;

GRANT EXECUTE ON FUNCTION public.claim_due_notification_jobs(integer, text) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.claim_due_scheduled_notification_settings(integer) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.claim_due_catchup_notification_settings(integer) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.claim_due_sms_notification_settings(integer) TO authenticated, service_role;
